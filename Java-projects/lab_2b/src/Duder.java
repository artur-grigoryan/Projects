...
class Duder {
	...
	static double []v=new double[5];
	static double[]u=new double[5];
	static double[]k=new double[5];
	...
	static{
	 ...
	 int selectCycle(){
		int i=generator.nextInt(epsDen);
		if(i>=0 && i<=2){
			return generator.nextInt(5);
		}
		
		if(currentPath!=-1){
			if(u[maxIndex]<u[currentPath]){
				maxIndex=currentPath;
			}
		}
		return maxIndex;
	}
    public int selectAction() {
    	...
    	{	
    		...
    		if(nodePaths[currentPath].contains(currentNode)){
    			...
    			currentIndex=nodePaths[currentPath].indexOf(currentNode);
    			startTime=System.currentTimeMillis();
    			cycleReward=cycleReward+ payout-travelCost-tollCharge;  //currentUtilitiesforVisitingNodes[currentNode];
    		...
    		if(pathLength==paths[currentPath].size()-1){
    			...
    			if(endTime!=startTime)
    				v[currentPath]=v[currentPath]+cycleReward*1000/(endTime-startTime);
    			k[currentPath]=k[currentPath]+1;
    			if(k[currentPath]!=0)
    				u[currentPath]=v[currentPath]/k[currentPath];
    			t=t+1;
    			epsDen=t+6;
    			cycleReward=0;
    		}
